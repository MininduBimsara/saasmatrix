...
export default function HomepageClient({ initialReviews, initialTools }: HomepageClientProps) {
  const router = useRouter();
  const [selectedCategory, setSelectedCategory] = useState<string>("All");
  const [searchTerm, setSearchTerm] = useState<string>("");
  const [isLoading, setIsLoading] = useState<boolean>(initialReviews.length === 0 && initialTools.length === 0);
  const [activeReviews, setActiveReviews] = useState<Review[]>(initialReviews);
  const [activeTools, setActiveTools] = useState<Tool[]>(initialTools);
              ...
              <div className="mt-8 p-5 border border-rose-100 bg-rose-50/20 rounded-[28px] animate-fade-in space-y-4 max-w-2xl w-full text-left relative z-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-11 items-center gap-3">
                  ...
                  <div className="sm:col-span-5 bg-white p-4 rounded-2xl border border-slate-150 shadow-xs flex flex-col gap-1.5">
                    ...
                    <p className="text-[11px] text-slate-505 leading-snug line-clamp-2">
              ...
              <div className="flex items-center justify-between gap-6 overflow-x-auto scrollbar-none pb-2 -mx-4 px-4 sm:mx-0 sm:px-0">
                ...
                <div className="hidden lg:flex items-center gap-2 border-l border-slate-200 pl-4">
                  <button
                    ...
                    className="flex items-center gap-1.5 bg-white border border-slate-205 rounded-xl px-4 py-2 text-xs font-bold text-slate-705 hover:shadow-xs transition-shadow cursor-pointer"
                  >
                ...
                {isLoading ? (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="text-xs font-black uppercase tracking-widest text-rose-500 hover:text-rose-605 transition-colors inline-block pt-1 cursor-pointer"
                    >
