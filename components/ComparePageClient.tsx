...
import { CATEGORIES, Tool, Review } from "@/lib/data";
import {
  Sparkles,
  ArrowRight,
  ArrowLeftRight,
...
export default function ComparePageClient({ initialTools, initialReviews }: ComparePageClientProps) {
  ...
  const defaultCatTools = defaultCategory
    ? initialTools.filter((t) => t.category === defaultCategory)
    : [];

  const [selectedCategory, setSelectedCategory] =
    useState<string>(defaultCategory);
  const [slugA, setSlugA] = useState<string>(defaultCatTools[0]?.slug ?? "");
  const [slugB, setSlugB] = useState<string>(defaultCatTools[1]?.slug ?? "");

  const [isSubscribed, setIsSubscribed] = useState(false);
  const [emailInput, setEmailInput] = useState("");
  const [isLoading, setIsLoading] = useState<boolean>(initialTools.length === 0 && initialReviews.length === 0);
  ...
  useEffect(() => {
    ...
    import("@/lib/contentSource").then(async (src) => {
      ...
      // Re-initialize selectors if we had empty initial list
      if (initialTools.length === 0 && publishedTools.length >= 2) {
        ...
        <main className="flex-grow py-16 px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl mx-auto rounded-3xl border border-slate-200 bg-slate-50 p-8 text-slate-800 shadow-sm">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Link
                href="/"
                className="inline-flex items-center rounded-full border border-slate-300 px-5 py-2.5 text-xs font-bold uppercase tracking-widest text-slate-705"
              >
                Back Home
              </Link>
            </div>
    ...
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <main id="interactive-compare-root" className="flex-grow py-12 font-sans">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-sm text-slate-650 max-w-3xl mb-12">
                  ...
                  <div className="divide-y divide-slate-100">
                    ...
                    {(toolAPricingModel || toolBPricingModel) && (
                      <div className="grid grid-cols-3 p-4 items-center gap-2">
                        ...
                        <div className="text-slate-650 capitalize">
                          {toolAPricingModel ?? "—"}
                        </div>
                        <div className="text-slate-655 capitalize">
                          {toolBPricingModel ?? "—"}
                        </div>
                      </div>
                    )}
                      ...
                      <div className="grid grid-cols-3 p-4 items-start gap-2">
                        ...
                        <ul className="space-y-1">
                          {toolAKeyFeatures.length > 0 ? toolAKeyFeatures.map((f, i) => (
                            <li key={i} className="text-[11px] text-slate-650 flex items-start gap-1">
                        ...
                        <ul className="space-y-1">
                          {toolBKeyFeatures.length > 0 ? toolBKeyFeatures.map((f, i) => (
                            <li key={i} className="text-[11px] text-slate-655 flex items-start gap-1">
                              <span className="text-emerald-500 mt-0.5 shrink-0">•</span>
                              {f}
                            </li>
                          )) : <li className="text-slate-400 text-[11px]">—</li>}
              ...
              <div className="bg-slate-50 border border-slate-200 p-6 md:p-8 rounded-2xl">
                <div className="max-w-xl">
                  <div className="inline-flex items-center gap-1 text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2 font-mono">
                    <Bell className="h-3.5 w-3.5 text-blue-505" />
