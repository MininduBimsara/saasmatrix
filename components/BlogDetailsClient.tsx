...
export default function BlogDetailsClient({ staticPost, slug }: BlogDetailsClientProps) {
  ...
  useEffect(() => {
    import('@/lib/clientDb').then((db) => {
      const allPosts = db.getMergedBlogPosts();
    ...
    return (
      <div className="flex flex-col min-h-screen bg-white">
        ...
        <main className="flex-grow py-24 text-center space-y-4 font-sans">
          <h2 className="text-xl font-bold text-slate-900">Dispatch File Not Found</h2>
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            The requested procurement analytical sheet has not been indexed yet or lives in a different directory.
          </p>
          <Link href="/blog" className="inline-block text-xs font-bold text-blue-600 hover:underline">
  ...
  const textBlocks = post.contentMarkdown ? post.contentMarkdown.split('\n\n') : [];
      ...
      <main id="blog-post-root" className="flex-grow py-12 font-sans bg-white">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <nav className="mb-6 flex items-center gap-1 text-xs text-slate-400 font-mono">
            <Link href="/" className="hover:text-blue-605">HOME</Link> / 
            <Link href="/blog" className="hover:text-blue-605 uppercase ml-1">THE DISPATCH</Link> / 
            <span className="truncate max-w-[200px] font-medium text-slate-500 ml-1 font-mono">ISSUE #{post.issueNumber}</span>
          </nav>
          ...
          <header className="mb-10 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <span className="flex items-center gap-1 text-xs font-mono text-slate-400">
                ...
                {post.readTime} &bull; {post.publicationDate}
              </span>
            </div>
          ...
          <AdContainer layoutType="top-banner" slotId={`blog-${post.slug}-above-post`} />

          {/* Prose body (narrow 680px visual width) */}
          <section id="prose-body" className="prose prose-slate prose-sm md:prose-base max-w-prose text-slate-700 leading-relaxed my-8 space-y-5">
            {textBlocks.map((block: string, idx: number) => {
              ...
              if (cleanBlock.startsWith('###')) {
                ...
                return (
                  <div key={idx}>
                    ...
                    {idx === 4 && (
                      <div className="my-6">
                        <AdContainer layoutType="inline-content" slotId={`blog-${post.slug}-inline-ad`} />
                      </div>
                    )}
                  </div>
                );
              ...
              if (cleanBlock.startsWith('1.') || cleanBlock.startsWith('2.') || cleanBlock.startsWith('3.')) {
                // Numbered lists
                const items = cleanBlock.split('\n');
                return (
                  <ol key={idx} className="list-decimal pl-5 space-y-2 my-4 text-sm leading-relaxed text-slate-600">
                    {items.map((item, idy) => {
              ...
              return (
                <p key={idx} className="text-sm md:text-base leading-relaxed text-slate-600 text-justify">
                  {matches.map((chunk, isOdd) => {
                    if (isOdd % 2 === 1) {
                      return <strong key={isOdd} className="font-extrabold text-slate-900">{chunk}</strong>;
                    }
                    return chunk;
                  })}
                </p>
            ...
            <div id="visual-peak-quote" className="p-6 my-8 border-l-4 border-amber-500 bg-amber-50/50 rounded-r-xl">
